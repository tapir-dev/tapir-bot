...
- `event::ReactionEvent` and `BackendObserver::reaction` — the seam for using
  emoji reactions as signals (approvals, acks). The engine never acts on a
  reaction; the consumer decides what it means.
